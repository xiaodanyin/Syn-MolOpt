...
def crossover(parents, offspring_size, distribution="even"):
    ...
    inherit_num = np.where(
        ...
        inherit_num,
        int(fp_length / 5) * np.ones((offspring_size,)),
    )
    inherit_num = np.where(
    ...
    for k in range(offspring_size):
        parent1_idx = list(
            set(np.random.choice(fp_length, size=int(inherit_num[k]), replace=False))
        )
        ...
        if distribution == "even":
            ...
            parent_set = parents[
                np.random.choice(parents.shape[0], size=2, replace=False, p=p_ / np.sum(p_))
            ]
        elif distribution == "softmax_linear":
            p_ = np.arange(parents.shape[0])[::-1] + 10
            parent_set = parents[
                np.random.choice(
                    parents.shape[0], size=2, replace=False, p=scipy.special.softmax(p_)
                )
            ]
...
def mutation(offspring_crossover, num_mut_per_ele=1, mut_probability=0.5):
    ...
    for idx in range(offspring_crossover.shape[0]):
        ...
        if mut_proba[idx] <= mut_probability:
            position = np.random.choice(fp_length, size=int(num_mut_per_ele), replace=False)
