...
def SMEG_hyperopt(times, task_name, max_evals=30, classification=False):
    ...
    args['bin_path'] = '../../data/substructure_analysis/graph_data/' + args['data_name'] + '.bin'
    args['group_path'] = '../../data/substructure_analysis/graph_data/' + args['data_name'] + '_group.csv'
    args['times'] = times
    ...
    for time_id in range(args['times']):
        ...
        stop_test_list, _ = run_an_eval_epoch(args, model, test_loader, loss_criterion,
                                               out_path='../../outputs/substructure_analysis/mol/' + args['task_name'] + '_' + pred_name + '_test')
        stop_train_list, _ = run_an_eval_epoch(args, model, train_loader, loss_criterion,
                                                out_path='../../outputs/substructure_analysis/mol/' + args['task_name'] + '_' + pred_name + '_train')
        stop_val_list, _ = run_an_eval_epoch(args, model, val_loader, loss_criterion,
                                              out_path='../../outputs/substructure_analysis/mol/' + args['task_name'] + '_' + pred_name + '_val')
    ...
    with open('../../outputs/substructure_analysis/result/hyperparameter_{}.pkl'.format(task_name), 'wb') as f:
        pkl.dump(args, f, pkl.HIGHEST_PROTOCOL)
    result_pd.to_csv('../../outputs/substructure_analysis/result/SMEG_' + args['task_name'] + '_all_result.csv', index=False)
